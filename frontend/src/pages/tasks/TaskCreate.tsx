...
import {
  ArrowLeft, Flag, Tag, Search, CheckCircle, X,
} from "lucide-react";

const TASK_TYPES = [
  { value: "GENERAL", label: "General", color: "bg-gray-100 text-gray-700" },
  { value: "FOLLOW_UP_CALL", label: "Follow-up Call", color: "bg-blue-100 text-blue-700" },
  { value: "EMBRYO_DISPOSAL", label: "Embryo Disposal", color: "bg-rose-100 text-rose-700" },
  { value: "GAMETE_REMINDER", label: "Gamete Reminder", color: "bg-amber-100 text-amber-700" },
  { value: "EXPIRY_NOTIFICATION", label: "Expiry Notification", color: "bg-purple-100 text-purple-700" },
] as const;
      ...
      <form onSubmit={handleSubmit}>
        <div className="space-y-6">
            ...
            <Card>
              ...
              <div className="flex flex-wrap gap-2">
                {TASK_TYPES.map((t) => (
                  ...
                  >
                    <span className={`inline-block w-2 h-2 rounded-full mr-1.5 ${t.color.split(" ")[0]}`} />
                    {t.label}
                  </button>
                ))}
              </div>
          ...
          <Card>
            ...
            {loadingStaff ? (
              ...
              <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 text-sm text-amber-700">
                Unable to load staff list. You may need admin permissions to view staff. Please type the assignee ID manually or contact an admin.
              </div>
            ) : (
              <select
                value={assigneeId}
            ...
            {!staffError && staff.length === 0 && !loadingStaff && (
              <p className="text-xs text-amber-600 mt-1">
                No staff available. Contact an admin to register staff accounts.
              </p>
            )}
          </Card>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <Button type="submit" isLoading={saving} icon={<CheckCircle className="w-4 h-4" />}>
